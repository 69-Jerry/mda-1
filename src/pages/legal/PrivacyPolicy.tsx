      ...
      <section className="py-40 px-8 relative z-20">
        <div className="max-w-5xl mx-auto space-y-12">
          {sections.map((section, i) => (
            <div
              key={i}
              className="glass-card bg-white p-12 lg:p-16 rounded-[4rem] border-white shadow-[0_40px_100px_-20px_rgba(93,26,26,0.05)] flex flex-col md:flex-row gap-12 items-start group hover:-translate-y-2 transition-all duration-700"
            >
              <div className="w-20 h-20 rounded-3xl bg-mda-maroon flex items-center justify-center shrink-0 shadow-xl transition-transform group-hover:scale-110">
                <section.icon className="text-mda-pink" size={36} />
              </div>
              <div className="space-y-6">
                <h2 className="text-4xl font-display text-mda-maroon uppercase tracking-tighter">
                  {section.title}
                </h2>
                <p className="font-body text-mda-dark/50 leading-relaxed text-xl border-l border-mda-pink/10 pl-8 italic">
                  {section.content}
                </p>
              </div>
            </div>
          ...
          <div className="pt-24 text-center">
            <div className="inline-block p-12 glass-card rounded-[3rem] border-mda-maroon/5 space-y-4">
              <p className="font-body text-sm text-mda-dark/40 uppercase tracking-[0.2em]">
                For systematic clarifications regarding this shield:
              </p>
              <a
                href="mailto:info@mdagh.org"
                className="text-2xl font-display text-mda-maroon uppercase italic hover:text-mda-pink transition-colors"
              >
                security@mdagh.org
              </a>
            </div>
